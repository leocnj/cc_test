...
class TextClassificationDataset(Dataset):
    ...
    def __init__(
        ...
        self.data["text"] = (
            self.data["title"].fillna("") + " " + self.data["description"].fillna("")
        )
...
def create_stratified_split(
    csv_path: str, train_ratio: float = 0.8, random_seed: int = 42
    ...
    train_df, val_df = train_test_split(
        df, test_size=1 - train_ratio, random_state=random_seed, stratify=df["tag"]
    )
